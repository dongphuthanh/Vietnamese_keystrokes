...
VOTING_MODE = 'soft'  
SCENARIO = sys.argv[1] if len(sys.argv) > 1 else 'M5'
OPTUNA_TRIALS = 0
OPTUNA_EPOCHS = 30    
FINAL_EPOCHS = 100
...
for fold in range(n_splits):
    ...
    study = optuna.create_study(direction="maximize",study_name=study_name, sampler=sampler, storage="sqlite:///optuna_studies/typenet_cie.db", load_if_exists=True)
...
plt.tight_layout()
plt.savefig(f"figures/typenet_cie_{SCENARIO}_confusion_matrix.png", dpi=300, bbox_inches='tight')
plt.close()

# Combined (all folds) confusion matrix, row-normalized %, in the same layout as the paper figures
combined_cm = confusion_matrix(all_true_labels, all_pred_labels, labels=[0, 1, 2, 3, 4])
combined_pct = 100 * combined_cm / combined_cm.sum(axis=1, keepdims=True)
print("")
print(f"Combined confusion matrix ({SCENARIO}, rows = true, cols = B P T F_P F_T, %):")
for name, row in zip(["B", "P", "T", "F_P", "F_T"], combined_pct):
    print(f"{name:>4} " + " ".join(f"{v:6.1f}" for v in row))