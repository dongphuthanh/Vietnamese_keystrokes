...
def get_key_id_6class(current_key_str, prev_key_str=""):
    ...
    # --- TELEX DIACRITIC HEURISTICS ---
    if len(ck) == 1:
        # 1. Double-Tap Modifiers (â, ê, ô, đ)
        if ck in {'a', 'e', 'o', 'd'} and ck == pk:
            is_diacritic = True
            
        # 2. Hook Modifiers (ă, ơ, ư)
        elif ck == 'w' and pk in {'a', 'o', 'u'}:
            is_diacritic = True
            
        # 3. Tone Modifiers (sắc, huyền, hỏi, ngã, nặng)
    ...
    # --- THE 6-CLASS ROUTING ---
    if is_diacritic:
        return 4                      # Class 4: Diacritic
        
    if len(ck) == 1:
        if ck.isalpha():
            return 0                  # Class 0: Normal Character
        elif ck.isdigit():
            return 1                  # Class 1: Digit
        elif ck == ' ':
            return 2                  # Class 2: Space
            
    if ck == 'backspace':
        return 3                      # Class 3: Backspace
    ...
    for (label, q_id), keys in grouped_keys.items():
        ...
        for i in range(1, len(keys) - win_length, stride):
            ...
            for j in range(i, i + win_length):
                ...
                key_id = get_key_id_6class(keys[j]["key"], keys[j - 1]["key"])
...
for fold in range(n_splits):
    ...
    for target_label in targets_to_inspect:
        ...
        # Find the first file ID that matches the current target label
        ...
        if target_fid is not None:
            ...
            for i, p in enumerate(window_probs):
                # Places an asterisk next to whichever class is winning this specific window
