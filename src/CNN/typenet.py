...
def get_key_id_full(key_str):
    """
    Maps standard ASCII and JS special keys to integers (0-255).
    Assumes high-Unicode characters have already been preprocessed out.
    """
    # 1. Catch the multi-character JavaScript control strings
    special_keys = {
        "Backspace": 8, "Tab": 9, "Enter": 13, "Shift": 16, 
        "Control": 17, "Alt": 18, "CapsLock": 20, "Escape": 27, 
        " ": 32, "PageUp": 33, "PageDown": 34, "End": 35, 
        "Home": 36, "ArrowLeft": 37, "ArrowUp": 38, 
        "ArrowRight": 39, "ArrowDown": 40, "Insert": 45, 
        "Delete": 46, "Meta": 91, "ContextMenu": 93
    }

    if key_str in special_keys:
        return special_keys[key_str]

    # 2. Because you preprocessed, we can safely assume it's a standard length-1 string
    if len(key_str) == 1:
        return ord(key_str)

    # 3. Absolute fallback just in case a weird string slipped through
    return 0
...
def get_key_id_from_code(keycode_str):
    ...
    # Class 0: Alphabetic letters (e.g., 'keya', 'keyz')
    if keycode_str.startswith('key'):
        return 0
        
    # Class 3: Numbers (e.g., 'digit1', 'numpad5')
    elif keycode_str.startswith('digit') or keycode_str.startswith('numpad'):
        return 3
        
    # Class 1: Spacebar
    elif keycode_str == 'space':
        return 1
        
    # Class 2: Backspace
    elif keycode_str == 'backspace':
        return 2
...
def make_windows(filepath, X, Y, ID, FILE_ID, Q_ID, user_id, file_id, win_length, stride):
    ...
    # Step 1: Group ALL events (both keydown and keyup) by Question ID
    grouped_events = {}
    for k in _keystrokes:
        if k["event"] in ["keydown", "keyup"]:
            ...
            if label is not None:
                ...
                if group_key not in grouped_events: grouped_events[group_key] = []
                grouped_events[group_key].append(k)

    # Step 2: Pair Press and Release times to create full Keystrokes
    for (label, q_id), events in grouped_events.items():
        pending_presses = {}
        paired_keys = []
        ...
        for ev in events:
            ...
            if ev["event"] == "keydown" and char not in pending_presses:
                ...
                r_time = ev["timestamp"]
                # Assuming your get_key_id maps to 0-255 ASCII ranges
        ...
        # i starts at 1 so we can look backwards at [j-1] for IL, PL, and RL
        for i in range(1, len(paired_keys) - win_length + 1, stride):
            ...
            for j in range(i, i + win_length):
                ...
                # Clip at 0 slightly to prevent impossible negative times from bad hardware
                ...
                # 3. TypeNet Keycode Normalization
                norm_key = curr['key'] / 255.0
                
                # 4. Form the 1x5 vector for this specific timestep
...
for fold in range(n_splits):
    ...
    study = optuna.create_study(direction="maximize",study_name=study_name, sampler=sampler, storage="sqlite:///koreantypenet128.db", load_if_exists=True)
