...
# ---------------------------------------------------------
# 3. HYBRID ARCHITECTURE (TRANSFORMER -> CNN)
# ---------------------------------------------------------
...
# *** MAKE SURE TO UPDATE THIS PATH IF NEEDED ***
...
# ---------------------------------------------------------
# 6. NESTED CROSS-CONTENT OPTUNA EVALUATION LOOP
# ---------------------------------------------------------
...
for fold in range(n_splits):
    ...
    def objective(trial):
        ...
        lr_transformer = trial.suggest_float('lr_transformer', 1e-5, 1e-4, log=True)
        lr_cnn = trial.suggest_float('lr_cnn', 5e-4, 5e-3, log=True)
        ...
        for epoch in range(OPTUNA_EPOCHS):
            ...
            for xb, yb in t_loader:
                ...
                loss.backward()
                optimizer.step()  
                scheduler.step() # CRITICAL: Inside the batch loop
    ...
    # Extract Optuna's chosen parameters
    best_lr_trans = best_params['lr_transformer']
    best_lr_cnn = best_params['lr_cnn']
    ...
    optimizer = torch.optim.AdamW(param_groups_final, weight_decay=1e-4)
    criterion = nn.CrossEntropyLoss()
    
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, 
        max_lr=[best_lr_trans, best_lr_cnn], 
    ...
    for epoch in range(FINAL_EPOCHS):
        ...
        for xb, yb in train_loader:
            ...
            # CRITICAL: Step inside the batch loop!
            scheduler.step()
...
# ---------------------------------------------------------
# 8. PLOT THREE SEPARATE CONFUSION MATRICES
# ---------------------------------------------------------
