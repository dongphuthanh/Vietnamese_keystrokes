...
# ---------------------------------------------------------
# 1. MODIFIED WINDOW GENERATOR (WITH QUESTION PARSING)
# ---------------------------------------------------------
def make_windows(filepath, X, Y, ID, FILE_ID, Q_ID, user_id, file_id, win_length, stride):
    ...
    # Grouping dynamically by (label, q_id)
    grouped_keys = {}
    ...
    for k in _keystrokes:
        if k["event"] == "keydown":
            ...
            # Parse format "Question.Session" (e.g., "3.2" -> q_id=3, s_id=2)
            ...
            if not is_attack:
                # Genuine Users
                if s_id == 1: label = 0     # B  (Bonafide)
                elif s_id == 2: label = 1   # P  (Paraphrase)
                elif s_id == 3: label = 2   # T  (Transcribe)
            else:
                # Attackers (Driven strictly by Session, allowing all Q_IDs to pass)
                if s_id == 2: label = 3     # F_P (Fake Paraphrase)
                elif s_id == 3: label = 4   # F_T (Fake Transcribe)
    ...
    # Extract windows for each valid group
...
print("Generating windows and parsing Question Indexes...")
file_counter = 0 
...
# ---------------------------------------------------------
# 3. DETERMINISTIC CROSS-CONTENT EVALUATION LOOP
# ---------------------------------------------------------
...
# Loop through our 3 specific Question Groups
n_splits = len(QUESTION_GROUPS)

for fold in range(n_splits):
    # The Test set is the current group
    test_questions = QUESTION_GROUPS[fold]
    # The Train set is the other two groups combined
    ...
    # Apply Scenario Filter (ONLY to the training set)
    ...
    with torch.no_grad():
        for xb, yb in test_loader:
            ...
            for j in range(len(store_vals)):
                # THE FIX: Group by (File ID, Session Label)
                # This perfectly merges Q4 and Q6 together if they share the same session/label, 
                # but keeps Session 1 and Session 2 completely separate.
    ...
    for (fid, true_lab), vals_list in session_results.items():
        if VOTING_MODE == 'soft':
            ...
            # Fallback for EER if hard voting is used
            all_pred_probs.append(np.zeros(5)) 
    ...
    # Calculate accuracy for this specific fold
...
# ---------------------------------------------------------
print("-" * 30)
print(f"Scenario {SCENARIO} Average Zero-Shot Content Accuracy: {np.mean(fold_accuracies):.4f} (+/- {np.std(fold_accuracies):.4f})")
print("-" * 30)
