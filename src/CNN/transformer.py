...
# ---------------------------------------------------------
# NEW: TEMPORAL TRANSFORMER ARCHITECTURE
# ---------------------------------------------------------
class TemporalTransformer(nn.Module):
    def __init__(self, feature_dim, num_classes=5, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, dropout=0.2):
        super().__init__()
        
        # 1. Project raw features (3 dims) to the Transformer's hidden size (64 dims)
        self.input_proj = nn.Linear(feature_dim, d_model)
        
        # 2. Positional Encoding (Learnable) so the model knows the order of keystrokes
        # Supports up to a window length of 1000
        self.pos_encoder = nn.Parameter(torch.randn(1, 1000, d_model)) 
        
        # 3. The Core Transformer Encoder
    ...
    def forward(self, x):
        ...
        # Global Average Pooling: Compress the 200 keystrokes into a single session vector
        x = x.mean(dim=1) 
...
for fold, (train_idx, test_idx) in enumerate(gkf.split(X, Y, groups=ID)):
    ...
    model = TemporalTransformer(feature_dim=X.shape[2], num_classes=5).to(device)
