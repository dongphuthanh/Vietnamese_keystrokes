    ...
    for root, _, files in os.walk(user_path):
        for filename in files:
            if filename.endswith(".json"):
                ...
                gin_make_windows(filepath, X_raw, Y_raw, ID_raw, user_id, WIN_LENGTH, STRIDE)
...
all_graphs_base = make_burst_meta_graph(X_raw, Y_raw, ID_raw)
plot_meta_graph_distributions(all_graphs_base)
...
def init_weights_kaiming(m):
    if isinstance(m, torch.nn.Linear):
        # Use Kaiming Normal for ReLU-based networks
        # nonlinearity='relu' automatically adjusts the gain
...
for fold, (train_idx, test_idx) in enumerate(gkf.split(indices, Y_np, groups=ID_np)):
    ...
    # Directly use train/test indices from GroupKFold
    ...
    # --- 1. PREPARE SCALERS ---
    # Node features are now 5D: [Avg Speed, Std, Length, Dwell, LoneFlag]
    # Edge features are now 2D: [Log Pause, PauseFlag]
    node_scaler = RobustScaler()
    edge_scaler = RobustScaler()

    # --- 2. FIT SCALERS (Training Data Only) ---
    # Extract Node Features for fitting
    # We ignore the 'LoneFlag' (index 4) and 'Length' (index 2) usually, 
    # but scaling 'Length' can help if bursts vary wildly.
    all_train_nodes = np.concatenate([g.x.numpy() for g in train_graphs], axis=0)
    node_scaler.fit(all_train_nodes)

    # Extract Edge Features for fitting
    # We focus on scaling the Log Pause (index 0). We skip PauseFlag (index 1) as it's binary.
    all_train_edges = np.concatenate([g.edge_attr.numpy() for g in train_graphs], axis=0)
    edge_scaler.fit(all_train_edges)

    # --- 3. APPLY SCALING TO TRAIN & TEST ---
    for g_set in [train_graphs, test_graphs]:
        for g in g_set:
            # Scale Edge Attributes (2D)
            # We keep the flags as they are, but transform the log-time
            g.edge_attr = torch.tensor(edge_scaler.transform(g.edge_attr.numpy()), dtype=torch.float32)
            
            # Scale Node Attributes (5D)
            # This handles Avg Speed, Std, Length, and Dwell simultaneously
    ...
    model = BurstMetaGINE(node_in_dim=4, edge_in_dim=4, hidden_dim=HIDDEN_DIM, num_classes=3).to(device)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch:02d} | Train Loss: {train_loss/len(train_loader):.4f}")

    # --- Evaluation Phase ---
    model.eval()
    session_results = {}
    correct_windows, total_windows = 0, 0
    ...
    print(f"Fold {fold+1} Window Acc: {f_window_acc:.4f} | Session Acc: {f_session_acc:.4f}")
