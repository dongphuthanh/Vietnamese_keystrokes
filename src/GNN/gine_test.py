...
all_graphs_base = gin_make_graph_skip_backbone(X_raw, Y_raw, ID_raw)
plot_all_topological_distributions_with_flag(all_graphs_base)
...
for fold, (train_idx, test_idx) in enumerate(gkf.split(indices, Y_np, groups=ID_np)):
    ...
    has_dwell_time = train_graphs[0].x.shape[1] > 1
    ...
    if has_dwell_time:
        ...
        all_train_dwell = np.concatenate([g.x[:, 1:2].numpy() for g in train_graphs], axis=0)
        node_scaler.fit(all_train_dwell)
    ...
    for g_set in [train_graphs, test_graphs]:
        for g in g_set:
            ...
            if has_dwell_time:
                dwell_scaled = node_scaler.transform(g.x[:, 1:2].numpy())
                flags = g.x[:, 0:1].numpy()
                g.x = torch.tensor(np.hstack((flags, dwell_scaled)), dtype=torch.float32)
            else:
                g.x = g.x.to(torch.long)
    ...
    model = KeystrokeGINE(num_node_types=3, edge_in_dim=4, hidden_dim=HIDDEN_DIM, num_classes=3).to(device)
