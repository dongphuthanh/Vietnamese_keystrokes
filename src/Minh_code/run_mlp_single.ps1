...
Write-Host "   STARTING MLP TRAINING FOR SCENARIO: $m (3 FOLDS) " -ForegroundColor Green
...
foreach ($f in $folds) {
    ...
    $out_dir = "runs\mlp_${m}_fold${f}"
    
    Write-Host "`n[+] Running: MLP | Scenario: $m | Fold: $f" -ForegroundColor Yellow
    Write-Host "Train file: $train_file" -ForegroundColor Gray
    Write-Host "Test file : $test_file" -ForegroundColor Gray
    
    # Execute Python script for MLP
    python MLP.py --train $train_file --test $test_file --outdir $out_dir --feature-percentage 50 --seed 42 --cm-labels 0 1 2 3 4
}

Write-Host "`n=================================================" -ForegroundColor Green
Write-Host "   SUCCESSFULLY COMPLETED 3 FOLDS FOR $m (MLP)!  " -ForegroundColor Green
Write-Host "=================================================" -ForegroundColor Green